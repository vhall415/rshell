...
vector<char *> alphaOrder(const char* start){
	vector<char *> newvec;
...
void printa(const char *dirnm, bool listall){
	struct stat sb;
	bool hidden = false;
	vector<char *> filesvec = alphaOrder(dirnm);
	for(unsigned i = 0; i < filesvec.size(); ++i){
		...
		delete [] path; //added to delete mem assigned in alphaOrder
		delete [] filesvec.at(i);
	}
	cout << endl;
}

int rflagTot(const char *path, bool listall){
	int total = 0;
	struct stat sb;
...
void printl(const char *dirnm, bool listall){
	struct stat sb;
	bool hidden = false;
	cout << "total " << rflagTot(dirnm, listall) << endl;
	vector<char *> filesvec = alphaOrder(dirnm);
	...
	for(unsigned i = 0; i < filesvec.size(); ++i){
		...
		delete [] path;
		delete [] filesvec.at(i); //deletes mem allocated in alphaOrder
	}
}

void printRecurse(const char *dirnm, bool listall){ 
	struct stat sb;
	string case1 = ".";
	string case2 = "..";
	bool hidden = false;
	...
	for(unsigned i = 0; i < filesvec.size(); ++i){
		...
		delete [] filesvec.at(i); //alphaOrder mem dealloc
	}
	cout << endl;
	while(!additionalpaths.empty()){
		...
		additionalpaths.pop();
		printRecurse(next.c_str(), listall);
	}
}
...
void printlRecurse(const char *dirnm, bool listall){
	struct stat sb;
	string case1 = ".";
	string case2 = "..";
	bool hidden = false;
	...
	cout << "total " << rflagTot(pathh, listall) << endl;
	...
	while(!additionalpaths.empty()){
		...
		delete[] additionalpaths.front();
		additionalpaths.pop();
		printlRecurse(next.c_str(), listall);
	}
}
